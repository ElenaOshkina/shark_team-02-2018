...
@Service
    ...
    @Override
    public void handle(@NotNull Message message, @NotNull Integer userId) throws HandleExeption {
        final MessageHandler<?> messageHandler = handlerMap.get(message.getClass());
        if (messageHandler == null) {
            throw new HandleExeption("No handler for message of " + message.getClass().getName() + " type");
        }
    ...
    @Override
    public <T extends Message> void registerHandler(@NotNull Class<T> myClass, MessageHandler<T> handler) {
        handlerMap.put(myClass, handler);
    }
}
